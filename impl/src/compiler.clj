...
(defn- next-token
  ...
  (let [id-pattern (Pattern/compile "[a-zA-Z_][a-zA-Z0-9_]*")
    ...
    (cond
      ...
      {:type :stringConstant :data
       (-> (.findInLine sc (Pattern/compile "\".*?\""))
           (str/replace "\"" "")
           (str/replace " { " "{")
           (str/replace " } " "}")
           (str/replace " [ " "[")
...
(defn- token-data-upper? [{:keys [type data]}]
  (and (= :identifier type)
...
(defn- check-tokens
  "usage: (check-tokens [{:type :s :data 1} {:type :b :data 2}]
   ...
   (let [varTable (or useVarTable
                      (cond (in-status? :in-subroutine) subroutineVarTable
                            ...
                            :else (throw (RuntimeException.
                                           "set-var! not belong class or subroutine"))))
         name (if (map? name) (:data name) name)
...
(defn- compileClassVarDec!
  ...
  (let [sfNode (pop-ts!) typeNode (pop-ts!) varNode (pop-ts!)]
    (if
      (check-tokens [sfNode typeNode varNode]
      ...
      (let [nextVars (doall
                       (take-while (fn [[splitNode var2Node]]
                                     (if (check-tokens [splitNode var2Node]
                                                       (type-and? :symbol ",")
                                                       (type? :identifier))
                                       true
            ...
            endNode (pop-ts!)]
        (when (check-tokens [endNode] (type-and? :symbol ";"))
...
(defn- compileParameterList!
  ...
  (let [varTypeNode (pop-ts!)
        varNameNode (pop-ts!)]
    (if (check-tokens [varTypeNode varNameNode]
                      [:or (type-and-contains? :keyword "int" "char" "boolean")
                       (type? :identifier)]
                      (type? :identifier))
      (let [nextTypeVars
            (doall
              take-while
              ((fn [[splitNode varType2Node varName2Node]]
                 (if (check-tokens [splitNode varType2Node varName2Node]
                                   (type-and? :symbol ",")
...
(defn- compileVarDec!
  ...
  (let [varTypeNode (pop-ts!)
        typeNode (pop-ts!)
        varNameNode (pop-ts!)]
    (if (check-tokens [varTypeNode typeNode varNameNode]
                      (type-and? :keyword "var")
                      [:or (type-and-contains?
                             :keyword "int" "char" "boolean") (type? :identifier)]
                      (type? :identifier))
      (let [nextVars (doall (take-while
                              (fn [[splitNode var2Node]]
                                (if (check-tokens [splitNode var2Node]
                                                  (type-and? :symbol ",")
                                                  (type? :identifier))
                                  true
            ...
            endNode (pop-ts!)]
        (if (check-tokens [endNode] (type-and? :symbol ";"))
          ;更新符号表：标记某类已使用、某变量已定义，type 为其类型
          (do
...
(defn- compileDo!
  ...
  额外压入 this 并增加参数计数。" []
  (let [doNode (pop-ts!)
        {c1Type :type :as varNameOrSubroutineName} (pop-ts!)
    ...
    (if-not (= :identifier c1Type)
      ...
      (cond
        ...
        (let [;更新子程序名：func/methodName
              _ (set-var! varNameOrSubroutineName
                          {:kind :subroutine :is-using true}
                          classVarTable)
              leftB (pop-ts!)
              ...
              rightB (pop-ts!)
              endNode (pop-ts!)]
          (if (and (check-tokens [leftB rightB endNode]
                                 (type-and? :symbol "(")
                                 (type-and? :symbol ")")
                                 (type-and? :symbol ";"))
        ...
        (let [point (pop-ts!)
              ...
              isFirstClass? (token-data-upper? varNameOrSubroutineName)
              ...
              rightB (pop-ts!)
              endNode (pop-ts!)]
          (if (and (check-tokens [point subName leftB rightB endNode]
                                 (type-and? :symbol ".")
                                 (type? :identifier)
                                 (type-and? :symbol "(")
                                 (type-and? :symbol ")")
...
(defn- compileLet!
  ...
  (let [letNode (pop-ts!)
    ...
    (if (check-tokens [letNode varNameNode equalOrLeftMidBruceNode]
      ...
      (if (= (:data equalOrLeftMidBruceNode) "=")
        ...
        (let [expressionRes (compileExpression!)
          ...
          (if (and (not (nil? expressionRes))
                   (check-tokens [endNode] (type-and? :symbol ";")))
            (cache-nested-cmds-return-ast!
        ...
        (let [expressionRes (compileExpression!)
          ...
          (if (and expressionRes
                   expression2Res
                   (check-tokens [rightMiddleBruceNode equalNode endNode]
                                 (type-and? :symbol "]")
                                 (type-and? :symbol "=")
                                 (type-and? :symbol ";")))
            (cache-nested-cmds-return-ast!
...
(defn- compileWhile!
  ...
  (let [mark (-> (random-uuid) str (str/split #"-") first)
    ...
    (if (and (check-tokens [while leftBruce rightBruce
...
(defn- compileReturn!
  ...
  (let [return (pop-ts!)
    ...
    (if (check-tokens [return] (type-and? :keyword "return"))
...
(defn- compileIf!
  ...
  (let [mark (-> (random-uuid) str (str/split #"-") first)
        ...
        rightBigB (pop-ts!)]
    (if (and (check-tokens [ifNode leftB rightB
                            leftBigB rightBigB]
                           (type-and? :keyword "if")
                           (type-and? :symbol "(")
                           (type-and? :symbol ")")
      ...
      (if-not (let [{:keys [data type]} (peek-ts)]
        ...
        (let [elseNode (pop-ts!)
              ...
              rightBigB2 (pop-ts!)]
          (if (and (check-tokens [elseNode leftBigB2 rightBigB2]
...
(defn- compileStatements!
  "编译语句 statement*, 不包括 {}, 可能为空, 空返回 nil
  生成的 VM 指令保存在 cache-cmd 中" []
  (if-let [children (doall
                      (take-while
                        (comp not nil?)
                        (repeatedly compileStatement!)))]
    (cache-nested-cmds-return-ast!
      (mapv #(pop-temp! %) children)
...
(defn- compileSubroutine!
  ...
  (let [typeTypeNode (pop-ts!)
        ...
        return
        (if (check-tokens [typeTypeNode returnTypeNode
          ...
          (let [is-constructor? (= (:data typeTypeNode) "constructor")
                ...
                ;更新子程序符号表，类型 type 为 f/c/m
                _ (set-var! subroutineNameNode
                            {:kind :subroutine
                             :type (:data typeTypeNode)}
                            classVarTable)
                _ (when is-method?
                    (set-var! "this" {:kind :argument
                ...
                ;var 声明可能为 nil
                allVarDecRes (doall (take-while (comp not nil?)
                                                (repeatedly compileVarDec!)))
                statementsRes (compileStatements!)          ;statementsRes 可能为 nil
                statementsCmds (pop-temp! statementsRes)
                rightBigBruceNode (pop-ts!)
                ;写入命令到 vm-cmds
                _ (write-formats!
                    (flatten [(format funcNameCmd (count-subroutine-varTable-var))
                              initThisCmds
                              statementsCmds]))]
            (if (check-tokens [leftBigBruceNode rightBigBruceNode
                               rightSmallBruceNode]
                              (type-and? :symbol "{") (type-and? :symbol "}")
                              (type-and? :symbol ")"))
...
(defn- compileClass!
  ...
  (let [classNode (pop-ts!)
        ...
        leftBigBruceNode (pop-ts!)
        return
        (if (check-tokens [classNode classNameNode leftBigBruceNode]
                          (type-and? :keyword "class")
                          :identifier :symbol)
          ;更新当前 className 标记，添加 className 符号
          (let [_ (do
                    (set-var! classNameNode {:kind :class
                                             :type (:data classNameNode)}
                              classVarTable)
                    (set-class-name! (:data classNameNode)))
                all-classVarDec (doall (take-while (comp not nil?)
                                                   (repeatedly compileClassVarDec!)))
                all-subroutineDec (doall (take-while (comp not nil?)
                                                     (repeatedly compileSubroutine!)))
                rightBigBruceNode (pop-ts!)]
            (-> [:class]
  ...
  (let [{:keys [type data] :as next-check} (peek-ts)]
    (cond
      ...
      (let [leftBruce (pop-ts!)
            ...
            rightBruce (pop-ts!)]
        (if (and (not (nil? expressionRes))
                 (check-tokens [leftBruce rightBruce]
                               (type-and? :symbol "(")
                               (type-and? :symbol ")")))
          (cache-nested-cmds-return-ast!
          ...
          (cond
            ...
            (let [_ (set-var-careful! varNameOrSubroutineName {:is-using true})
                  ...
                  finalCmds (flatten [expCmds
                                      (format "push %s %d"
                                              (id-kind->vm-str kind) index)
                                      "add"
                                      "pop pointer 1"
                                      "push that 0"])
                  rightBB (pop-ts!)]
              (if (and (check-tokens [leftBB rightBB]
                                     (type-and? :symbol "[")
                                     (type-and? :symbol "]"))
                       (not (nil? expressionRes)))
                (cache-nested-cmds-return-ast!
            ...
            (let [_ (set-var! varNameOrSubroutineName {:kind     :subroutine
                  ...
                  rightB (pop-ts!)]
              (if (and (check-tokens [leftB rightB]
                                     (type-and? :symbol "(")
                                     (type-and? :symbol ")"))
            ...
            (let [point (pop-ts!)
                  ...
                  ;在调用前压入 expressionList 计算结果
                  ;如果是 method，则额外扩增参数长度，并压入 this
                  is-first-class? (token-data-upper? varNameOrSubroutineName)
                  ...
                  final-cmds (flatten
                               [(if is-second-method?
                                ...
                                (format "call %s.%s %d"
                                        ...
                                        paramCount)])
                  rightB (pop-ts!)]
              (if (and (check-tokens [point subName leftB rightB]
                                     (type-and? :symbol ".")
                                     (type? :identifier)
                                     (type-and? :symbol "(")
                                     (type-and? :symbol ")"))
            ...
            (let [return [:term (token->node varNameOrSubroutineName)]
              ...
              (set-temp! return [(format "push %s %d"
                                         (id-kind->vm-str kind) index)])
              return)))))))
...
(defn- compileExpression!
  ...
  (let [termRes (compileTerm!)
        ...
        isOP? #(check-tokens [%] (type-and-contains?
                                   :symbol "+" "-" "*" "/"
                                   "&" "|" "<" ">" "="))]
    (let [return
          (if (not (nil? termRes))
...
(defn- compileExpressionList!
  ...
  eg. f.m(10,20,10,10+30), 注意 expression 结果可能相同,
  temp-cmds hash 会冲突" []
    ...
    (if (not (nil? expRes))
      (let [nextExpFunc (fn [] (let [{:keys [type data] :as a} (pop-ts!)]
            ...
            nextExps (doall
                       (take-while
                         (fn [[sp expResInner]]
                           (if (not (nil? expResInner))
                             true (do (push-ts! sp) nil)))
                         (repeatedly nextExpFunc)))
            nextExpNodesRes (reduce (fn [agg [sp exp]]
                                      (conj agg (token->node sp) exp))
                                    [] nextExps)            ;may nil
            restExpCmds (reduce (fn [agg [_ exp]]
                                  (conj agg (get-temp exp)))
                                [] nextExps)
            ;防止 expression 相同以至于 pop 部分表达式指令丢失的问题，先 get 后 pop
            _ (doall (mapv #(pop-temp! (second %)) nextExps))
...
;2022-6-27
;实现了 IR 前端词法和语法分析部分，生成 AST
;2022-6-28
;实现了 IR 前端语法分析部分的 Debug 和测试程序测试
;2022-6-29
;实现了符号表生成、Debug 和其测试程序测试
;2022-6-30
;实现了简单程序和部分类型语句的 IR 前端代码生成(Square)
;2022-7-1 上午
;实现字符串字面量翻译，解决写入符号表时 kind 丢失问题，
;statements/statement：let√,if√,while√,do√,return√ 缓存结果而不直接写入命令，
;以解决 while 和 if 非线性写入命令的问题。
;2022-7-1 下午
...
;发现了基于 hash 的 cache-cmds 容易导致错误，比如参数列表两个相同的字面量 0，hash 一致
;修复了 letStatement 中 field 变量被错误更新为 var 类型的问题
;修复了 letStatement 中 expression - term 压入类变量对象调用方法时的错误
;2022-7-4 上午
;修复 temp-cmds 中相同 AST Node 和相同 hash 的相同命令弹出一次后丢失的问题
;优化代码结构，删除不必要的逻辑判断、注释，重命名副作用方法